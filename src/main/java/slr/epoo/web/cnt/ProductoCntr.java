...
public class ProductoCntr extends ControllerBase<Producto, String, ProductoDaoV2>{
    ...
    public @ResponseBody
    String listProductos() throws IOException{
        return jsonWrite(get());
    }

    @RequestMapping(value = "/{cod}", method = RequestMethod.GET, headers = {"Accept=Application/JSON"})
    public @ResponseBody
    String searchProducto(@PathVariable String cod) throws IOException{
    ...
    public @ResponseBody
    String insertProducto(@PathVariable String codigo, @RequestParam(value = "descripcion") String descripcion, @PathVariable BigDecimal precio) throws IOException{
        return jsonWrite(put(new Producto(codigo, descripcion, precio)));
    }

    @RequestMapping(value = "/{cod}", method = RequestMethod.DELETE, headers = {"Accept=Application/JSON"})
    public @ResponseBody
    String deleteProducto(@PathVariable String cod) throws IOException{
